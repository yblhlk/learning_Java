...
public class TestFile {
    public static void main(String[] args) throws IOException {
        ...
        // 3.打印目录
        {
            System.out.println("======================3.打印目录====================");
            File path = new File("D:\\JavaCode" + File.separatorChar +"OPP\\Day9\\src");
            File directory = new File(path, "\\Directory");
            printDirectoty(directory);
        }

    }

    // 3.打印目录 : 使用静态变量 + 循环来控制缩进
    static int sp_const = 1;
    public static void printDirectoty(File file) {
        // 递归终止条件
        ...
        if(file.isDirectory()) {
            ...
            for (File f : files) {
                // 控制缩进
                for (int i = 0; i < sp_const; i++) {
                    System.out.print(" ");
                }
                System.out.print((char) (sp_const + 48) + ">");
                sp_const++;
                printDirectoty(f);
                sp_const--;
            }
