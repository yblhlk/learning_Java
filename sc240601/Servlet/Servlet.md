...
>  >
>  > ![image-20240801085532263](D:\Desktop\Servlet\img\使用taglib指令.png)
...
> ```
>
> ![image-20240803170123709](D:\Desktop\Servlet\img\使用jsp发送数据给前端页面.png)
...
> * 4.根据控制层返回成功和失败
>
> ![image-20240801135946402](D:\Desktop\Servlet\img\通过jsp实现注册功能流程图.png)
...
> 2. 转发(必须写相对路径)
> response.getRequestDispatcher("../相对于当前jsp页面的地址").forward(request,response);
> ```
>
...
>
> HTTP请求状态码（HTTP Status Code）是由三位数字组成的代码，用于表示HTTP请求是否成功完成以及请求的结果状态。这些状态码由RFC 2616规范定义，并得到后续规范的扩展。HTTP状态码可以分为五大类：信息响应（100–199）、成功响应（200–299）、重定向（300–399）、客户端错误（400–499）和服务器错误（500–599）。
>
...
- 重写doGet()  doPost()  ...
  ...
  >
  > ![image-20240812072259486](D:\Desktop\Servlet\img\重写doGet方法.png)
...
### 5.转发(forward)和重定向(redirect)的区别 --- 面试题

> 请求重定向:将原来的请求重新定位到新的地址。
>
> ![image-20240729065047500](D:\Pictures\servlet\转发和重定向.png)
...
//servlet转发
request.getRequestDispatcher("").forward(request,response);   //springmvc框架转发
...
- 应用场景:   转发只能访问内部资源 和 WEB-INF的资源
  ...
  总结： 一般情况下 查询(使用最多的)场景适合转发(使用最多的)
...
> 服务器会为某个客户端的每次会话创建一个sessionId，然后保存在客户端自己的浏览器的Cookie中，所以是没有办法通过Cookie来进行不同客户端之间的数据通信的，因为每个sessionId都是客户端私有的。
 ...
 ![image-20240730154848628](D:\Pictures\servlet\Session原理.png)
...
> 进阶：如果我想要记录用户信息怎么办？
>
> ![image-20240808212919590](D:\Desktop\Servlet\img\用Cookie记住用户的账号和密码.png)
...
![img](D:\Pictures\servlet\过滤器的工作原理.png)
...
![image-20240803155357001](D:\Desktop\Servlet\img\使用Junit要导入junit依赖.png)
