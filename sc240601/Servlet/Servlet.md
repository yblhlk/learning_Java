...
> ```
>
> ![image-20240803170123709](.\img\使用jsp发送数据给前端页面.png)
...
>   > 重写doGet方法获取前端数据：
>   >
>   > ![image-20240812072259486](.\img\重写doGet方法.png)
>   >
>   > 重写doGet方法发送数据给前端：
...
> 1.HttpServlet实现类中：
> 	// a.先获取ServletContext对象(application作用域)通过HttpServletRequest对象来获取
...
>
> ```xml
> <!--实现servlet必做的第三步：配置web.xml文件-->
> <servlet>
>     <!--必须和一个以上的servlet-mapping标签对应通过servlet-name属性来对应-->
>     <servlet-name>servlet1</servlet-name>
>     <!--通过全类名来找到控制层类，实现控制层和要处理的请求的绑定-->
...
> 服务器会为某个客户端的每次会话创建一个sessionId，然后保存在客户端自己的浏览器的Cookie中，所以是没有办法通过Cookie来进行不同客户端之间的数据通信的，因为每个sessionId都是客户端私有的。
 ...
 ![image-20240730154848628](.\img\Session原理.png)
...
>
>```xml
>  <!--过滤器配置在全局配置下面，servlet前面-->
>  <!--配置编码过滤器，在全局配置下面，servlet前面-->
>  <filter>
>    <filter-name>encodingFilter</filter-name>
>    <filter-class>com.sc.filter.EncodingFilter</filter-class> <!--指定过滤器的完整类名,哪个类具体实现了过滤器-->
>    <!--过滤器中也能使用初始化参数和全局参数-->
>    <init-param> <!--请求和响应的编码规则，只需要过滤器使用一次就可以了，所以设置为Filter的初始化参数更合理-->
...
>    </init-param>
>  </filter>
>  <!--filter-mapping决定了过滤器执行的顺序，创建和销毁的顺序是随机的-->
>  <filter-mapping>
>    <filter-name>encodingFilter</filter-name>
>    <!--编码过滤器，要对所有资源进行过滤-->
>    <url-pattern>/*</url-pattern>
...
>         System.out.println("\033[35m【" + time + "】 监听到session作用域创建了新属性：" + name + ", 值为：" + value +
>                 "。\033[0m");
>     }
> 
>     //监听删除数据
